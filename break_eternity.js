...
//END pad-end
	...
	Allows for extremely large numbers
	...
	*/
	...
	var TOWER_LIMIT = Number.MAX_SAFE_INTEGER; //Number.MAX_SAFE_INTEGER
	...
	class BigDecimal { 
		...
		normalize() {
            if (this.p < 0) return Number.NaN;
			if (this.t >= 1 && this.t < 10) return this;
			while (this.t >= 10) {this.t = Math.log10(this.t);this.p++}
			while (this.t < 1) {this.t = 10**this.t;this.p--}
			return this;
		}
		
		fromMulti(p, t) {
			//SAFETY: don't let in non-numbers
			if (!Number.isFinite(t) || !Number.isFinite(p)) { t = Number.NaN; p = Number.NaN; }
			this.t = t;
			this.p = p;
		...
		fromNumber(value) {
			...
			if (Number.isNaN(value) || value < 0) { this.t = Number.NaN; this.p = Number.NaN; }
			else if (value == Number.POSITIVE_INFINITY) { this.t = 9.99999999999; this.p = TOWER_LIMIT; }
		...
		fromString(value) {
			...
			{
				var parts = value.split("PT");
				this.p = parseFloat(parts[0]);
				this.t = parseFloat(parts[1]);
			}
			else if (value == "NaN") { this.mantissa = Number.NaN; this.exponent = Number.NaN; }
			else
			{
				this.fromNumber(parseFloat(value));
				if (Number.isNaN(this.p)) { throw Error("[DecimalError] Invalid argument: " + value); }
			}
			return this.normalize();
		}
		
		fromValue(value,t) {
            if (args.length > 1) return this.fromMulti(value,t)
			if (value instanceof BigDecimal) return this.fromCopy(value);
			else if (typeof(value) == 'number') return this.fromNumber(value);
		...
		static fromMantissaExponent_noNormalize(mantissa, exponent) {
			return new BigDecimal().fromMantissaExponent_noNormalize(mantissa, exponent);
		}
		
		static fromDecimal(value) {
			return new BigDecimal().fromDecimal(value);
		}
		...
		static fromValue(value) {
			...
			return new BigDecimal().fromValue(value);
		}
        
        static new(value) { return BigDecimal.fromValue(value) }
		...
		toNumber() {
			...
			if (!Number.isFinite(this.p)) return this.p;
			if (this.p >= 2) return Number.POSITIVE_INFINITY;
            let p = this.p;
            let t = this.t;
            while (p-- > 0) t = 10**t;
			return t;
		...
		toString(places) {
			if (Number.isNaN(this.p) || Number.isNaN(this.t)) return "NaN";
			if (this.t >= TOWER_LIMIT) return "Infinity";
            if (!Number.isFinite(places)) places = MAX_SIGNIFICANT_DIGITS;
			return this.p + "PT" + this.t.toString().slice(0,places+2)
		}
		...
		toEs(places) {
			...
			if (this.t >= TOWER_LIMIT) return "Infinity";
            if (!Number.isFinite(places)) places = MAX_SIGNIFICANT_DIGITS;
			return padEnd("",this.p,"e") + this.t.toString().slice(0,places+2)
		}
		...
		round() {
			if (this.p == 0 || (this.p == 1 && this.t <= MAX_SIGNIFICANT_DIGITS)) return BigDecimal.fromNumber(Math.round(this.toNumber()));
			return this;
		}
		
		static round(value) { return BigDecimal.new(value).round(); }
		
		floor() {
			if (this.p == 0 || (this.p == 1 && this.t <= MAX_SIGNIFICANT_DIGITS)) return BigDecimal.fromNumber(Math.floor(this.toNumber()));
			return this;
		}
		
		static floor(value) { return BigDecimal.new(value).floor(); }
		
		ceil() {
			if (this.p == 0 || (this.p == 1 && this.t <= MAX_SIGNIFICANT_DIGITS)) return BigDecimal.fromNumber(Math.ceil(this.toNumber()));
			return this;
		}
		
		static ceil(value) { return BigDecimal.new(value).ceil(); }
		
		trunc() {
			if (this.p == 0 || (this.p == 1 && this.t <= MAX_SIGNIFICANT_DIGITS)) return BigDecimal.fromNumber(Math.trunc(this.toNumber()));
			return this;
		}
